...
export default function ClipOfTheDay() {
  ...
  useEffect(() => {
    ...
    const end = start + 3; 
    try { playerRef.current.seekTo(start); } catch (e) {}
    const interval = setInterval(() => {
      const player = playerRef.current;
      if (player && typeof player.getCurrentTime === 'function') {
        const curr = player.getCurrentTime();
        if (curr > end || curr < start) player.seekTo(start);
      }
    }, 100); 
  ...
  const checkAnswer = (selectedSong: any) => {
    ...
    if (guesses.some(g => g.type === 'clip' && g.text === `${selectedSong.artist.name} - ${selectedSong.title}`)) {
  ...
  const onPlayerReady = (event: any) => {
    ...
    if (target) event.target.seekTo(target.timestamps[round]);
  };
  ...
  const opts = {
    ...
    playerVars: { autoplay: 1, controls: 0, modestbranding: 1, rel: 0, showinfo: 0, mute: 1, start: target ? target.timestamps[round] : 0 },
  };
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-zinc-950 text-white p-4 font-sans relative pb-24">
      ...
      >
            ...
            {/* NAPRAWIONA SEKCJA HISTORII */}
            {guesses.length > 0 && (
                <div className="w-full flex flex-col gap-2 animate-fade-in">
                    {guesses.map((g, i) => (
                        <div key={i} className="flex items-center gap-3 p-3 rounded bg-zinc-950 border-l-4 border-red-500">
                            {g.type === 'clip' ? (
                                <>
                                    <div className="flex-1 min-w-0 opacity-60">
                                        <div className="text-sm font-bold truncate text-zinc-300 line-through">{g.text}</div>
                                    </div>
            ...
            {gameStatus === 'playing' && (
                <div className="w-full relative">
                    <input type="text" value={query} onChange={e => setQuery(e.target.value)} placeholder="Search for a video..." 
                        className="w-full p-4 rounded-lg bg-zinc-950 border border-zinc-700 focus:border-indigo-500 focus:outline-none text-white placeholder-zinc-500 transition-all"
                    />
                    {results.length > 0 && (
                    ...
                    <button onClick={handleSkip} className="w-full mt-3 py-3 text-xs font-bold tracking-widest text-zinc-500 hover:text-white hover:bg-zinc-800 rounded border border-zinc-800 transition-colors uppercase">SKIP (+ CLIP)</button>
                </div>
            )}
            ...
            {gameStatus !== 'playing' && target && (
                <div className="text-center w-full animate-fade-in pt-4 border-t border-zinc-800">
                    ...
                    <button onClick={handleShare} className="mt-6 w-full py-3 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-lg font-bold text-white hover:scale-105 transition-transform shadow-lg flex items-center justify-center gap-2"><span>📤</span> SHARE RESULT</button>
                    <Countdown />
                </div>
            )}
      </GameWrapper>
